...
export interface IHostSession extends Document {
  ...
  seatings: Array<{ id: string; serverId: string; tableId: string; partySize: number; startedAt: Date; completedAt?: Date; status: 'seated' | 'completed' | 'cancelled'; }>;
...
const SeatingSchema = new Schema<{ id: string; serverId: string; tableId: string; partySize: number; startedAt: Date; completedAt?: Date; status: 'seated' | 'completed' | 'cancelled'; }>({
