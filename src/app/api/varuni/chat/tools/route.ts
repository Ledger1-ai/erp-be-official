...
// It's extracted to be reusable between the chat and tools endpoints.
export function initializeAgent(): VaruniAgent {
  ...
  // The content of this function is a copy of the tool registration from `chat/route.ts`
  ...
  if (isOn('inventory', true)) register({
    ...
    tools: [
      ...
      createGraphQLTool(
        ...
        `query{ lowStockItems{ id name category currentStock minThreshold unit costPerUnit status } }`
      ),
      {
  ...
  if (isOn('analytics', true)) register({
    ...
    tools: [
      createGraphQLTool(
        'get_analytics',
        'Fetch dashboard analytics summary for a period (daily, weekly, monthly). Returns KPIs such as revenue, orders, avg order value, satisfaction, inventory value, etc.',
        `query($period:String!){ analytics(period:$period){ revenue orders avgOrderValue customerSatisfaction tableTurnover totalCustomers repeatCustomers averageWaitTime staffUtilization inventoryValue wastePercentage } }`
      ),
      createGraphQLTool(
  ...
  if (isOn('vendors', true)) register({
    ...
    tools: [
      createGraphQLTool(
        'get_vendors',
        'List vendors with key attributes, delivery info, performance metrics, and contacts.',
        `query{ vendors{ id name companyName supplierCode type categories status notes isPreferred deliveryInfo{deliveryDays deliveryWindow minimumOrder leadTimeDays} performanceMetrics{onTimeDeliveryRate qualityRating totalOrders totalSpent} contacts{ name title email phone mobile isPrimary } } }`
      ),
      { name: 'back_to_main', description: 'Return to main tools', handler: async () => ({ navigate: 'main' }) },
    ]
  });
  ...
  if (isOn('menus', true)) register({
    ...
    tools: [
      ...
      createGraphQLTool(
        ...
        `query($restaurantGuid:String!,$guids:[String!],$multiLocationIds:[String!]){ menuItemStock(restaurantGuid:$restaurantGuid,guids:$guids,multiLocationIds:$multiLocationIds){ guid multiLocationId status quantity versionId } }`
      ),
      { name: 'back_to_main', description: 'Return to main tools', handler: async () => ({ navigate: 'main' }) },
    ]
  });
  ...
  if (isOn('gql', true)) register({
    ...
    tools: [
      ...
      createGraphQLTool('list_vendors', 'List vendors', `query{ vendors{ id name companyName supplierCode type categories status isPreferred contacts{ name title email phone mobile isPrimary } address{ street city state zipCode country } performanceMetrics{ totalOrders totalSpent averageOrderValue onTimeDeliveryRate qualityRating } } }`),
      createGraphQLTool('get_vendor', 'Get vendor by id', `query($id:ID!){ vendor(id:$id){ id name companyName supplierCode type status isPreferred notes } }`),
      ...
      createGraphQLTool('get_analytics', 'Get analytics summary by period', `query($period:String!){ analytics(period:$period){ revenue orders avgOrderValue customerSatisfaction tableTurnover totalCustomers repeatCustomers averageWaitTime staffUtilization inventoryValue wastePercentage } }`),
      createGraphQLTool('get_revenue_analytics', 'Get revenue analytics series', `query($startDate:Date!,$endDate:Date!){ revenueAnalytics(startDate:$startDate,endDate:$endDate){ period date revenue orders avgOrderValue } }`),
      ...
      createGraphQLTool('get_supplier_performance_report', 'Supplier performance', `query($startDate:Date!,$endDate:Date!){ supplierPerformanceReport(startDate:$startDate,endDate:$endDate){ supplierId supplierName totalOrders totalSpent averageOrderValue onTimeDeliveryRate qualityRating } }`),
      ...
      createGraphQLTool('get_indexed_menus', 'Indexed menus for a restaurant', `query($restaurantGuid:String!){ indexedMenus(restaurantGuid:$restaurantGuid){ restaurantGuid lastUpdated menus{ guid name description } modifierGroupReferences{ referenceId guid name } modifierOptionReferences{ referenceId guid name price } } }`),
      ...
      createGraphQLTool('get_menu_item_stock', 'Menu item stock', `query($restaurantGuid:String!,$guids:[String!],$multiLocationIds:[String!]){ menuItemStock(restaurantGuid:$restaurantGuid,guids:$guids,multiLocationIds:$multiLocationIds){ guid multiLocationId status quantity versionId } }`),
      ...
      createGraphQLTool('create_vendor', 'Create vendor', `mutation($input:CreateVendorInput!){ createVendor(input:$input){ id name companyName isPreferred } }`),
      createGraphQLTool('update_vendor', 'Update vendor', `mutation($id:ID!,$input:UpdateVendorInput!){ updateVendor(id:$id,input:$input){ id name status isPreferred } }`),
      createGraphQLTool('delete_vendor', 'Delete vendor', `mutation($id:ID!){ deleteVendor(id:$id) }`),
      ...
      createGraphQLTool('set_menu_visibility', 'Set menu visibility', `mutation($restaurantGuid:String!,$hiddenMenus:[String!],$hiddenGroups:[String!]){ setMenuVisibility(restaurantGuid:$restaurantGuid,hiddenMenus:$hiddenMenus,hiddenGroups:$hiddenGroups){ restaurantGuid hiddenMenus hiddenGroups } }`),
...
export async function GET(req: NextRequest) {
  try {
    ...
    const toolsetName = searchParams.get('toolset') || 'main';
    const toolDefinitions = agent.getToolDefinitions(toolsetName);
    return NextResponse.json(toolDefinitions);
  } catch (err: any) {
    console.error('Error getting tool definitions:', err);
